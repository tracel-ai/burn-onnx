...
3. **Read the codebase.** Understand the architecture and conventions already in place. PRs that
   ignore existing patterns will be rejected. If you are implementing or modifying an operator, read
   [`DEVELOPMENT-GUIDE.md`](DEVELOPMENT-GUIDE.md) first.
4. **Run validation.** Run `cargo xtask validate` before you submit. This runs formatting, linting,
...
- Follow the existing code style and project conventions. See
  [`DEVELOPMENT-GUIDE.md`](DEVELOPMENT-GUIDE.md) for architecture details and coding conventions.
...
- New operators need both unit and integration tests. Bug fixes must include a regression test.
