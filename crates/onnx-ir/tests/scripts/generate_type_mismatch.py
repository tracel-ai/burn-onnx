...
def create_type_mismatch_model():
    ...
    input_tensor = helper.make_tensor_value_info(
        "input_tensor", TensorProto.FLOAT, [2, 3]
    )
    ...
    nodes = [
        ...
        helper.make_node(
            "Add", ["input_tensor", "input_scalar"], ["temp1"], name="add_tensor_scalar"
        ),
        # Result * const_scalar (tensor * scalar)
        helper.make_node(
            "Mul", ["temp1", "const_scalar"], ["temp2"], name="mul_scalar"
        ),
        # Add tensor constant (tensor + tensor)
        helper.make_node(
            "Add", ["temp2", "const_tensor"], ["output"], name="add_tensor"
        ),
    ]
