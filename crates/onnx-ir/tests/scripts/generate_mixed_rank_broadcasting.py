...
def create_mixed_rank_model():
    ...
    input_2d = helper.make_tensor_value_info(
        "input_2d", TensorProto.FLOAT, [1, 3]
    )  # 2D
    input_scalar = helper.make_tensor_value_info(
        "input_scalar", TensorProto.FLOAT, []
    )  # Scalar

    # Outputs
    output1 = helper.make_tensor_value_info(
        "output_4d_scalar", TensorProto.FLOAT, [1, 3, 4, 4]
    )
    output2 = helper.make_tensor_value_info(
        "output_4d_2d", TensorProto.FLOAT, [1, 3, 4, 4]
    )
    ...
    nodes = [
        ...
        helper.make_node(
            "Add", ["input_4d", "input_scalar"], ["temp1"], name="add_4d_scalar"
        ),
        # temp1 * scalar_const (4D * 0D)
        helper.make_node(
            "Mul", ["temp1", "scalar_const"], ["output_4d_scalar"], name="mul_4d_scalar"
        ),
        # 4D + 2D broadcasting (different ranks)
        helper.make_node("Add", ["input_4d", "input_2d"], ["temp2"], name="add_4d_2d"),
        # temp2 + 1D constant (4D + 1D)
        helper.make_node(
            "Add", ["temp2", "const_1d"], ["output_4d_2d"], name="add_4d_1d"
        ),
    ]
