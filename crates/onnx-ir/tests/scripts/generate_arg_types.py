...
def create_arg_types_model():
    ...
    input_4d = helper.make_tensor_value_info(
        "input_4d", TensorProto.FLOAT, [1, 3, 4, 4]
    )  # 4D tensor
    input_2d = helper.make_tensor_value_info(
        "input_2d", TensorProto.FLOAT, [3, 5]
    )  # 2D tensor
    input_1d = helper.make_tensor_value_info(
        "input_1d", TensorProto.FLOAT, [10]
    )  # 1D tensor
    ...
    output_scalar = helper.make_tensor_value_info(
        "output_scalar", TensorProto.FLOAT, []
    )  # Scalar output
    output_reshaped = helper.make_tensor_value_info(
        "output_reshaped", TensorProto.FLOAT, [1, 3, 16]
    )  # Reshaped tensor
    ...
    nodes = [
        ...
        helper.make_node(
            "Mul", ["sum_result", "scalar_value"], ["output_scalar"], name="mul_scalar"
        ),
        # Reshape: Tensor + Shape → Tensor with new shape
        # This tests Shape argument type and constant lifting
        helper.make_node(
            "Reshape", ["input_4d", "target_shape"], ["output_reshaped"], name="reshape"
        ),
    ]
