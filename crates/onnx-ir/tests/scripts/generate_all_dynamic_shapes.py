...
def create_all_dynamic_shapes_model():
    ...
    input1 = helper.make_tensor_value_info(
        "input1", TensorProto.FLOAT, ["batch", "channels", "height", "width"]
    )
    input2 = helper.make_tensor_value_info(
        "input2", TensorProto.FLOAT, ["batch", "channels", "height", "width"]
    )

    # Output also has dynamic shape
    output = helper.make_tensor_value_info(
        "output", TensorProto.FLOAT, ["batch", "channels", "height", "width"]
    )
