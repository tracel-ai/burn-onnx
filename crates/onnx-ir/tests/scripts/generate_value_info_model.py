...
def create_model():
    ...
    input2 = helper.make_tensor_value_info(
        "input2", TensorProto.FLOAT, ["batch", 28, 28]
    )
    ...
    add1 = helper.make_node(
        "Add", inputs=["transpose1_out", "input2"], outputs=["add1_out"], name="add1"
    )
    ...
    reshape2 = helper.make_node(
        "Reshape", inputs=["add1_out", "shape2"], outputs=["output1"], name="reshape2"
    )

    # Output
    output1 = helper.make_tensor_value_info(
        "output1", TensorProto.FLOAT, ["batch", 784]
    )
    ...
    value_info = [
        ...
        helper.make_tensor_value_info(
            "reshape1_out", TensorProto.FLOAT, ["batch", 28, 28]
        ),
        # transpose1_out should be rank 3
        helper.make_tensor_value_info(
            "transpose1_out", TensorProto.FLOAT, ["batch", 28, 28]
        ),
