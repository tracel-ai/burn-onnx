...
pub mod alexnet {
    include!(concat!(env!("OUT_DIR"), "/model/alexnet.rs"));
}
...
fn main() {
    println!("========================================");
    println!("BVLC AlexNet Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("alexnet");
    ...
    println!("Initializing AlexNet model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/alexnet.bpk");
    let model: alexnet::Model = alexnet::Model::from_file(weights_path, &device);
    ...
    let input = test_data.data_0.val();
    let reference_output = test_data.prob_1.val();
