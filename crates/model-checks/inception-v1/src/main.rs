...
pub mod inception_v1 {
    include!(concat!(env!("OUT_DIR"), "/model/inception_v1.rs"));
}
...
fn main() {
    println!("========================================");
    println!("Inception v1 (GoogLeNet) Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("inception-v1");
    ...
    println!("Initializing Inception v1 model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/inception_v1.bpk");
    let model: inception_v1::Model = inception_v1::Model::from_file(weights_path, &device);
