...
pub mod vgg19 {
    include!(concat!(env!("OUT_DIR"), "/model/vgg19.rs"));
}
...
fn main() {
    println!("========================================");
    println!("VGG-19 Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("vgg19");
    ...
    println!("Initializing VGG-19 model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/vgg19.bpk");
    let model: vgg19::Model = vgg19::Model::from_file(weights_path, &device);
