...
pub mod zfnet512 {
    include!(concat!(env!("OUT_DIR"), "/model/zfnet512.rs"));
}

#[derive(Debug, Module)]
struct TestData {
    gpu_0_data_0: Param<Tensor<4>>,
    gpu_0_softmax_1: Param<Tensor<2>>,
}

impl TestData {
    fn new(device: &Device) -> Self {
        Self {
            gpu_0_data_0: Initializer::Zeros.init([1, 3, 224, 224], device),
            gpu_0_softmax_1: Initializer::Zeros.init([1, 1000], device),
        }
    }
}
...
fn main() {
    println!("========================================");
    println!("ZFNet-512 Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("zfnet512");
    ...
    println!("Initializing ZFNet-512 model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/zfnet512.bpk");
    let model: zfnet512::Model = zfnet512::Model::from_file(weights_path, &device);
    ...
    let input = test_data.gpu_0_data_0.val();
    let reference_output = test_data.gpu_0_softmax_1.val();
