...
fn main() {
    let artifacts = model_checks_common::artifacts_dir_build("arcface");
    let onnx_path = artifacts.join("arcface.onnx");
    ...
    if !onnx_path.exists() {
        ...
        eprintln!();
        eprintln!("This will download the ArcFace model and upgrade it to opset 16.");
        std::process::exit(1);
    }
