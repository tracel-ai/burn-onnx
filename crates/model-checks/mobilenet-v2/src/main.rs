...
pub mod mobilenet_v2 {
    include!(concat!(env!("OUT_DIR"), "/model/mobilenet_v2.rs"));
}
...
fn main() {
    println!("========================================");
    println!("MobileNet v2 Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("mobilenet-v2");
    ...
    println!("Initializing MobileNet v2 model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/mobilenet_v2.bpk");
    let model: mobilenet_v2::Model = mobilenet_v2::Model::from_file(weights_path, &device);
