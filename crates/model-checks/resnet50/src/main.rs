...
pub mod resnet50 {
    include!(concat!(env!("OUT_DIR"), "/model/resnet50.rs"));
}
...
fn main() {
    println!("========================================");
    println!("ResNet-50 Burn Model Test");
    println!("========================================\n");

    let artifacts_dir = model_checks_common::artifacts_dir("resnet50");
    ...
    println!("Initializing ResNet-50 model...");
    let start = Instant::now();
    let device = model_checks_common::best_device!();
    let weights_path = concat!(env!("OUT_DIR"), "/model/resnet50.bpk");
    let model: resnet50::Model = resnet50::Model::from_file(weights_path, &device);
