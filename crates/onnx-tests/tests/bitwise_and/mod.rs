...
mod tests {
    ...
    fn bitwise_and_scalar_tensor() {
        let device = Default::default();
        let model: bitwise_and_scalar::Model = bitwise_and_scalar::Model::new(&device);
        // Run the model
        let input1 = Tensor::<1, Int>::from_ints([1, 2, 3, 4], &device);
        let scalar = 2;
        let output = model.forward(input1, scalar);
        let expected = TensorData::from([0i32, 2, 2, 0]);
        output.to_data().assert_eq(&expected, true);
    }
    ...
    fn scalar_bitwise_and_tensor() {
        let device = Default::default();
        let model: scalar_bitwise_and::Model = scalar_bitwise_and::Model::new(&device);
        // Run the model
        let scalar = 2;
        let input2 = Tensor::<1, Int>::from_ints([1, 2, 3, 4], &device);
    ...
    fn scalar_bitwise_and_scalar() {
        let device = Default::default();
        let model: scalar_bitwise_and_scalar::Model =
