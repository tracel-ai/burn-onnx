...
def main():
    node0 = helper.make_node(
        ...
        value=numpy_helper.from_array(np.array([2.0], dtype=np.float32).reshape([]), name="value"))
    node1 = helper.make_node(
        "Add", ["onnx::Add_0", "/Constant_output_0"], ["2"])

    inp_onnx__Add_0 = helper.make_tensor_value_info("onnx::Add_0", TensorProto.FLOAT, [2, 3, 4])
