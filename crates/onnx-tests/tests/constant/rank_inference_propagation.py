...
def create_model():
    ...
    input_3d = helper.make_tensor_value_info(
        "input",
        TensorProto.FLOAT,
        [None, None, 384],  # Dynamic batch, sequence, features
    )
    ...
    const_shape = helper.make_tensor(
        ...
        vals=[
            8,
            16,
        ],  # Will be concatenated with sliced shape to form [batch, seq, 8, 16]
    )
    ...
    shape_node = helper.make_node(
        "Shape", inputs=["input"], outputs=["input_shape"], name="shape"
    )

    # Slice the shape to get only [batch, seq] dimensions
    # Create constants for slice parameters
    slice_starts = helper.make_tensor(
        name="slice_starts", data_type=TensorProto.INT64, dims=[1], vals=[0]
    )

    slice_ends = helper.make_tensor(
        name="slice_ends", data_type=TensorProto.INT64, dims=[1], vals=[2]
    )

    slice_starts_node = helper.make_node(
        "Constant", inputs=[], outputs=["starts"], value=slice_starts
    )

    slice_ends_node = helper.make_node(
        "Constant", inputs=[], outputs=["ends"], value=slice_ends
    )
    ...
    matmul1_node = helper.make_node(
        "MatMul", inputs=["input", "weight1"], outputs=["matmul1_out"], name="matmul1"
    )

    # Second MatMul - should output rank 3
    matmul2_node = helper.make_node(
        "MatMul", inputs=["input", "weight2"], outputs=["matmul2_out"], name="matmul2"
    )
    ...
    output = helper.make_tensor_value_info(
        "output",
        TensorProto.FLOAT,
        [None, None, 8, 16],  # Dynamic batch and sequence, fixed last dims
    )
