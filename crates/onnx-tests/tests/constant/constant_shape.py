...
def main():
    ...
    shape_node = helper.make_node("Shape", inputs=["input"], outputs=["input_shape"])
    ...
    scalar_const = helper.make_node(
        ...
        value=helper.make_tensor(
            name="scalar_tensor", data_type=TensorProto.INT64, dims=[], vals=[2]
        ),
    )
    ...
    add_scalar_node = helper.make_node(
        "Add", inputs=["input_shape", "scalar"], outputs=["shape_add_scalar"]
    )

    # Shape * Scalar
    mul_scalar_node = helper.make_node(
        "Mul", inputs=["input_shape", "scalar"], outputs=["shape_mul_scalar"]
    )

    # Shape + Shape
    add_shape_node = helper.make_node(
        "Add", inputs=["input_shape", "shape_constant"], outputs=["shape_add_shape"]
    )

    # Shape * Shape
    mul_shape_node = helper.make_node(
        "Mul", inputs=["input_shape", "shape_constant"], outputs=["shape_mul_shape"]
    )
