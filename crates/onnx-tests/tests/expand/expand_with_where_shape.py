...
def main():
    ...
    cos_node = helper.make_node("ConstantOfShape", ["const_3"], ["ones"],
    ...
    const_shape2 = helper.make_node(
        "Constant", [], ["target_shape2"],
        value=numpy_helper.from_array(np.array([2, 3, 4], dtype=np.int64), name="ts2"),
    )
    where_node = helper.make_node("Where", ["eq_out", "ones", "target_shape2"], ["expand_shape"])
    ...
    graph = helper.make_graph(
        [const_3, cos_node, const_neg1, mul_node, const_shape, equal_node,
         const_shape2, where_node, expand_node],
        "expand_where_test",
        [inp],
        [out],
    )
