...
def main():
    ...
    output = helper.make_tensor_value_info("output", TensorProto.INT64, [2])

    # Create reshape target to reshape from 4 elements to 2 elements [2, 2]
    reshape_target = numpy_helper.from_array(
        torch.tensor([2], dtype=torch.int64).numpy(), name="reshape_target"
    )

    nodes = [
        # Get shape of input (will be Shape(4) containing [2, 3, 4, 5])
        helper.make_node(
            "Shape", inputs=["input"], outputs=["input_shape"], name="shape1"
        ),
    ...
    start = numpy_helper.from_array(
        torch.tensor([0], dtype=torch.int64).numpy(), name="slice_start"
    )
    end = numpy_helper.from_array(
        torch.tensor([2], dtype=torch.int64).numpy(), name="slice_end"
    )
