...
# Generates: lstm_runtime_weights.onnx
# LSTM whose W/R/B arrive as graph inputs rather than initializers, the shape every
...
def main():
    ...
    w = helper.make_tensor_value_info(
        "W", TensorProto.FLOAT, [NUM_DIRECTIONS, 4 * HIDDEN_SIZE, INPUT_SIZE]
    )
    r = helper.make_tensor_value_info(
        "R", TensorProto.FLOAT, [NUM_DIRECTIONS, 4 * HIDDEN_SIZE, HIDDEN_SIZE]
    )
    ...
    graph = helper.make_graph([lstm_node], "lstm_runtime_weights_graph", [inp, w, r, b], [out_Y, out_Y_h])
    ...
    test_w = ramp([NUM_DIRECTIONS, 4 * HIDDEN_SIZE, INPUT_SIZE], 0.02, -0.4)
    test_r = ramp([NUM_DIRECTIONS, 4 * HIDDEN_SIZE, HIDDEN_SIZE], 0.015, -0.3)
    test_b = ramp([NUM_DIRECTIONS, 8 * HIDDEN_SIZE], 0.01, -0.1)
    ...
    onnx.save(model, "lstm_runtime_weights.onnx")
    print("Saved lstm_runtime_weights.onnx")
