...
def main():
    ...
    mat2d_square_input = helper.make_tensor_value_info(
        "mat2d_square", TensorProto.FLOAT, [4, 4]
    )

    # Test case 1: 2D × 1D (matrix-vector)
    matmul_2d_1d = helper.make_node(
        "MatMul", inputs=["mat2d", "vec"], outputs=["output_2d_1d"], name="matmul_2d_1d"
    )
    ...
    matmul_3d_1d = helper.make_node(
        "MatMul", inputs=["mat3d", "vec"], outputs=["output_3d_1d"], name="matmul_3d_1d"
    )
    ...
    mat3d_for_vec_input = helper.make_tensor_value_info(
        "mat3d_for_vec", TensorProto.FLOAT, [2, 3, 4]
    )
    ...
    output_3d_1d = helper.make_tensor_value_info(
        "output_3d_1d", TensorProto.FLOAT, [2, 3]
    )
    output_1d_3d = helper.make_tensor_value_info(
        "output_1d_3d", TensorProto.FLOAT, [2, 4]
    )
    output_2d_2d = helper.make_tensor_value_info(
        "output_2d_2d", TensorProto.FLOAT, [3, 4]
    )
