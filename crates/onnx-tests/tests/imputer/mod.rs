...
mod tests {
    ...
    fn imputer_sentinel_replacement() {
        ...
        let model: imputer::Model = imputer::Model::new(&device);

        // input: [[1.0, -999.0, 3.0], [4.0, 5.0, -999.0]]
        // -999.0 is replaced by 0.0 (imputed value)
        let input = Tensor::<2>::from_floats([[1.0f32, -999.0, 3.0], [4.0, 5.0, -999.0]], &device);
    ...
    fn imputer_per_feature_sentinel_replacement() {
        let device = Default::default();
        let model: imputer_per_feature::Model = imputer_per_feature::Model::new(&device);

        // input: [[-999.0, 2.0, -999.0], [4.0, -999.0, 6.0]]
        // per-feature imputed values: [10.0, 20.0, 30.0]
        let input =
            Tensor::<2>::from_floats([[-999.0f32, 2.0, -999.0], [4.0, -999.0, 6.0]], &device);
    ...
    fn imputer_int_sentinel_replacement() {
        let device = Default::default();
        let model: imputer_int::Model = imputer_int::Model::new(&device);

        // input: [[1, -1, 3], [4, 5, -1]]
        // -1 is the sentinel replaced by 0 (imputed value)
        // Note: Using i32 literals to match backend's default Int type
        let input = burn::tensor::Tensor::<2, burn::tensor::Int>::from_ints(
            [[1i32, -1, 3], [4, 5, -1]],
            &device,
        );
    ...
    fn imputer_nan_sentinel_replacement() {
        let device = Default::default();
        let model: imputer_nan::Model = imputer_nan::Model::new(&device);

        // input: [[1.0, NaN, 3.0], [4.0, 5.0, NaN]]
        // NaN is the explicit sentinel replaced by 0.0 (imputed value)
        let input =
            Tensor::<2>::from_floats([[1.0f32, f32::NAN, 3.0], [4.0, 5.0, f32::NAN]], &device);
    ...
    fn imputer_nan_default_sentinel_replacement() {
        let device = Default::default();
        let model: imputer_nan_default::Model = imputer_nan_default::Model::new(&device);
