...
def main():
    ...
    shape1 = helper.make_node("Shape", inputs=["input"], outputs=["shape1"])
    ...
    const_shape = helper.make_node(
        ...
        value=helper.make_tensor(
            name="const_value", data_type=TensorProto.INT64, dims=[3], vals=[2, 3, 4]
        ),
    )

    # Equal: Compare the two shapes
    equal = helper.make_node(
        "Equal", inputs=["shape1", "const_shape"], outputs=["output"]
    )
    ...
    graph = helper.make_graph(
        [shape1, const_shape, equal], "equal_shape_test", [input_tensor], [output]
    )
