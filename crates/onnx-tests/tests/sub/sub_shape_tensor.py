...
def main():
    ...
    sub_shape_tensor = helper.make_node(
        "Sub", inputs=["shape", "input_1d"], outputs=["shape_minus_tensor"]
    )

    # Subtract shape from tensor (Tensor - Shape)
    sub_tensor_shape = helper.make_node(
        "Sub", inputs=["input_1d", "shape"], outputs=["tensor_minus_shape"]
    )

    # Outputs
    output1 = helper.make_tensor_value_info(
        "shape_minus_tensor", TensorProto.INT64, [3]
    )
    output2 = helper.make_tensor_value_info(
        "tensor_minus_shape", TensorProto.INT64, [3]
    )
    ...
    assert np.array_equal(shape_minus_tensor, -tensor_minus_shape), (
        "Subtraction results should be negatives of each other"
    )
