...
def main():
    node0 = helper.make_node(
        "Sub", ["onnx::Sub_0", "onnx::Sub_0"], ["/Sub_output_0"])
    node1 = helper.make_node(
        "Constant", [], ["/Constant_output_0"],
        value=numpy_helper.from_array(np.array([9], dtype=np.int64).reshape([]), name="value"))
    ...
    node4 = helper.make_node(
        "Sub", ["/Sub_1_output_0", "/Sub_2_output_0"], ["6"])

    inp_onnx__Sub_0 = helper.make_tensor_value_info("onnx::Sub_0", TensorProto.INT64, [1, 1, 1, 4])
    inp_onnx__Sub_1 = helper.make_tensor_value_info("onnx::Sub_1", TensorProto.INT64, [])
