...
def main():
    ...
    cast_a0_u8 = helper.make_node(
        "Cast", ["a0_i32_out"], ["a0_u8"], to=TensorProto.UINT8
    )
    cast_b0_u8 = helper.make_node(
        "Cast", ["b0_i32_out"], ["b0_u8"], to=TensorProto.UINT8
    )

    # Rank cases (zp=0 for all)
    n_2d_1d = helper.make_node(
        "MatMulInteger",
        ["mat2d", "vec4", "a0_u8", "b0_u8"],
        ["y_2d_1d"],
        name="mmi_2d_1d",
    )
    n_1d_2d = helper.make_node(
        "MatMulInteger",
        ["vec4", "sq4", "a0_u8", "b0_u8"],
        ["y_1d_2d"],
        name="mmi_1d_2d",
    )
    n_3d_1d = helper.make_node(
        "MatMulInteger",
        ["mat3d", "vec4", "a0_u8", "b0_u8"],
        ["y_3d_1d"],
        name="mmi_3d_1d",
    )
    n_1d_3d = helper.make_node(
        "MatMulInteger",
        ["vec3", "mat3d_b", "a0_u8", "b0_u8"],
        ["y_1d_3d"],
        name="mmi_1d_3d",
    )
    n_2d_2d = helper.make_node(
        "MatMulInteger",
        ["mat2d", "sq4", "a0_u8", "b0_u8"],
        ["y_2d_2d"],
        name="mmi_2d_2d",
    )
    ...
    y_1d_3d_np = np.matmul(
        vec3_np.astype(np.int32), mat3d_b_np.astype(np.int32)
    )  # (2,4)
    ...
    print(
        "y_2d_1d verification:",
        "PASS" if np.array_equal(got_all[0], y_2d_1d_np) else "FAIL",
    )
    ...
    print(
        "y_1d_2d verification:",
        "PASS" if np.array_equal(got_all[1], y_1d_2d_np) else "FAIL",
    )
    ...
    print(
        "y_3d_1d verification:",
        "PASS" if np.array_equal(got_all[2], y_3d_1d_np) else "FAIL",
    )
    ...
    print(
        "y_1d_3d verification:",
        "PASS" if np.array_equal(got_all[3], y_1d_3d_np) else "FAIL",
    )
    ...
    print(
        "y_2d_2d verification:",
        "PASS" if np.array_equal(got_all[4], y_2d_2d_np) else "FAIL",
    )
