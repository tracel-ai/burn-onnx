...
def main():
    ...
    cast_a2_u8 = helper.make_node(
        "Cast", ["a2_i32_out"], ["a2_u8"], to=TensorProto.UINT8
    )
    cast_b3_u8 = helper.make_node(
        "Cast", ["b3_i32_out"], ["b3_u8"], to=TensorProto.UINT8
    )
    cast_a0_i8 = helper.make_node(
        "Cast", ["a0_i32_out"], ["a0_i8"], to=TensorProto.INT8
    )

    # ---- MatMulInteger nodes ----
    nA = helper.make_node(
        "MatMulInteger", ["A", "B", "a0_u8", "b0_u8"], ["YA"], name="mmi_u8u8_zp0"
    )
    nB = helper.make_node(
        "MatMulInteger", ["C", "D", "a2_u8", "b3_u8"], ["YB"], name="mmi_u8u8_zp_const"
    )
    nC = helper.make_node(
        "MatMulInteger", ["E", "F", "a0_i8", "b0_u8"], ["YC"], name="mmi_i8u8_zp0"
    )
    ...
    # YA
    got_YA = ref.run(
        ["YA"], {"A": A_np, "B": B_np, "C": C_np, "D": D_np, "E": E_np, "F": F_np}
    )[0]
    ...
    got_YB = ref.run(
        ["YB"], {"A": A_np, "B": B_np, "C": C_np, "D": D_np, "E": E_np, "F": F_np}
    )[0]
    ...
    got_YC = ref.run(
        ["YC"], {"A": A_np, "B": B_np, "C": C_np, "D": D_np, "E": E_np, "F": F_np}
    )[0]
