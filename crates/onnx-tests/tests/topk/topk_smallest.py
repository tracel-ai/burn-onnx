...
"""Generate TopK model selecting the smallest k=3 along axis 1 of a [3, 5] tensor."""
...
def main():
    ...
    graph = helper.make_graph([k_const, topk_node], "topk_smallest", [X], [Values, Indices])
    model = helper.make_model(graph, opset_imports=[helper.make_operatorsetid("", OPSET_VERSION)])
    onnx.checker.check_model(model)
    onnx.save(model, "topk_smallest.onnx")
