...
def main():
    node0 = helper.make_node("ReduceMean", ["onnx::ReduceMean_0"], ["1"], keepdims=0)
    node1 = helper.make_node("ReduceMean", ["onnx::ReduceMean_0"], ["2"], keepdims=0)
    node2 = helper.make_node(
        "ReduceMean", ["onnx::ReduceMean_0"], ["3"], axes=[1], keepdims=1
    )
    node3 = helper.make_node(
        "ReduceMean", ["onnx::ReduceMean_0"], ["4"], axes=[-1], keepdims=1
    )
    node4 = helper.make_node(
        "ReduceMean", ["onnx::ReduceMean_0"], ["5"], axes=[0], keepdims=0
    )
    node5 = helper.make_node(
        "ReduceMean", ["onnx::ReduceMean_0"], ["6"], axes=[0, 2], keepdims=0
    )

    inp_onnx__ReduceMean_0 = helper.make_tensor_value_info(
        "onnx::ReduceMean_0", TensorProto.FLOAT, [1, 1, 2, 4]
    )
