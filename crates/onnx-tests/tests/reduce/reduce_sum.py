...
def main():
    node0 = helper.make_node("ReduceSum", ["onnx::ReduceSum_0"], ["2"], keepdims=0)
    node1 = helper.make_node("ReduceSum", ["onnx::ReduceSum_0", ""], ["3"], keepdims=0)
    node2 = helper.make_node(
        "Constant",
        [],
        ["onnx::ReduceSum_4"],
        value=numpy_helper.from_array(
            np.array([1], dtype=np.int64).reshape([1]), name="value"
        ),
    )
    node3 = helper.make_node(
        "ReduceSum", ["onnx::ReduceSum_0", "onnx::ReduceSum_4"], ["5"], keepdims=1
    )
    ...
    node5 = helper.make_node(
        "ReduceSum", ["onnx::ReduceSum_0", "onnx::ReduceSum_6"], ["7"], keepdims=1
    )
    ...
    node7 = helper.make_node(
        "ReduceSum", ["onnx::ReduceSum_0", "onnx::ReduceSum_8"], ["9"], keepdims=0
    )
    node8 = helper.make_node(
        ...
        ["onnx::ReduceSum_10"],
        value=numpy_helper.from_array(
            np.array([0, 2], dtype=np.int64).reshape([2]), name="value"
        ),
    )
    node9 = helper.make_node(
        "ReduceSum", ["onnx::ReduceSum_0", "onnx::ReduceSum_10"], ["11"], keepdims=0
    )

    inp_onnx__ReduceSum_0 = helper.make_tensor_value_info(
        "onnx::ReduceSum_0", TensorProto.FLOAT, [1, 1, 2, 4]
    )
