...
def main():
    ...
    single_use_constant = helper.make_node(
        ...
        value=helper.make_tensor(
            name="single_use_tensor", data_type=TensorProto.FLOAT, dims=[], vals=[2.0]
        ),
    ...
    multi_use_constant = helper.make_node(
        ...
        value=helper.make_tensor(
            name="multi_use_tensor", data_type=TensorProto.FLOAT, dims=[], vals=[3.0]
        ),
    ...
    clip_min_constant = helper.make_node(
        ...
        outputs=["clip_min"],
        value=helper.make_tensor(
            name="clip_min_tensor", data_type=TensorProto.FLOAT, dims=[], vals=[0.0]
        ),
    ...
    clip_max_constant = helper.make_node(
        ...
        outputs=["clip_max"],
        value=helper.make_tensor(
            name="clip_max_tensor", data_type=TensorProto.FLOAT, dims=[], vals=[10.0]
        ),
    ...
    output_tensor = helper.make_tensor_value_info(
        "final_output", TensorProto.FLOAT, [2, 3]
    )
    ...
    model = helper.make_model(
        graph, producer_name="test", opset_imports=[helper.make_opsetid("", 16)]
    )

    # Save the model
    onnx.save(model, "constant_reused.onnx")
    print(
        "Generated constant_reused.onnx - Tests constant lifting with reused constants"
    )
    ...
    print(
        f"  4. output = clip(add, 0.0, 10.0) (single-use constants, should be lifted)"
    )
