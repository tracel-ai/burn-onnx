...
mod tests {
    ...
    fn quantize_linear() {
        ...
        let input = Tensor::<2>::from_floats([[-1.0, 0.0, 1.0, 2.1]], &device);
        let scale = Tensor::<1>::from_floats([0.5], &device);
        let zero_data = TensorData::from([3i16]);
        let zero_point = Tensor::<1, Int>::from_data(zero_data, (&device, DType::I16));
    ...
    fn quantize_linear_per_axis() {
        ...
        let input = Tensor::<2>::from_floats([[-1.0, 0.0, 1.0], [2.0, 4.0, 6.0]], &device);
