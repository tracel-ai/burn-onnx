...
def main() -> None:
    np.random.seed(42)
    model = create_model()
    onnx.save(model, "lp_pool2d_opset1.onnx")
    ...
    windows = np.abs(x.astype(np.float64)).reshape(1, 1, 2, 2, 2, 2).transpose(0, 1, 2, 4, 3, 5)
    expected = ((windows**1.5).sum(axis=(4, 5)) ** (1.0 / 1.5)).astype(np.float32)
    np.testing.assert_allclose(y, expected, rtol=1e-5)

    print("Saved lp_pool2d_opset1.onnx")
    print("Input:", x)
    print("Output:", y)
