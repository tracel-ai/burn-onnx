...
mod tests {
    ...
    fn argmax() {
        ...
        let model: argmax::Model = argmax::Model::default();
    ...
    fn argmax_both_keepdims() {
        ...
        let model: argmax_both_keepdims::Model = argmax_both_keepdims::Model::default();
        ...
        let input = Tensor::<2>::from_floats([[1.0, 3.0, 2.0], [4.0, 2.0, 1.0]], &device);
    ...
    fn argmax_select_last_index() {
        ...
        let model: argmax_select_last_index::Model = argmax_select_last_index::Model::default();

        let device = Default::default();
        // Row 0: max is 5.0 at indices 1 and 2 -> last is 2
        // Row 1: all tied at 3.0 -> last is 3
        let input = Tensor::<2>::from_floats([[1.0, 5.0, 5.0, 2.0], [3.0, 3.0, 3.0, 3.0]], &device);
        let output = model.forward(input);
    ...
    fn argmax_1d() {
        // Initialize the model with weights (loaded from the exported file)
        let model: argmax_1d::Model = argmax_1d::Model::default();

        let device = Default::default();
        // Run the model with test input [1.0, 3.0, 2.0, 5.0, 4.0]
        // Expected output: 3 (index of max value 5.0)
        let input = Tensor::<1>::from_floats([1.0, 3.0, 2.0, 5.0, 4.0], &device);
        let output = model.forward(input);
