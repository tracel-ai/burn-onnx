...
def create_gather_constant_2d_indices():
    ...
    data_init_1 = helper.make_tensor(
        "data_init_1", TensorProto.FLOAT, [5, 7], data_values.flatten().tolist()
    )

    indices_init_1 = helper.make_tensor(
        "indices_init_1", TensorProto.INT64, [1, 5], indices_values.flatten().tolist()
    )
    ...
    data_init_2 = helper.make_tensor(
        "data_init_2", TensorProto.FLOAT, [5, 7], data_values.flatten().tolist()
    )
    ...
    data_const_tensor = helper.make_tensor(
        "data_const_tensor", TensorProto.FLOAT, [5, 7], data_values.flatten().tolist()
    )
    ...
    indices_init_3 = helper.make_tensor(
        "indices_init_3", TensorProto.INT64, [1, 5], indices_values.flatten().tolist()
    )
    ...
    add_node_1 = helper.make_node(
        "Add", inputs=["output_1", "output_2"], outputs=["sum_12"], name="Add_1_2"
    )

    add_node_2 = helper.make_node(
        "Add", inputs=["sum_12", "output_3"], outputs=["output"], name="Add_final"
    )
