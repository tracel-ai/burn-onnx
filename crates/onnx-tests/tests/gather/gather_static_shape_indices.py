...
def create_test_with_constant_indices():
    ...
    gather_scalar = helper.make_node(
        "Gather", inputs=["data", "scalar_indices"], outputs=["output_scalar"], axis=0
    )

    gather_1d = helper.make_node(
        "Gather", inputs=["data", "indices_1d"], outputs=["output_1d"], axis=0
    )

    gather_2d = helper.make_node(
        "Gather", inputs=["data", "indices_2d"], outputs=["output_2d"], axis=0
    )

    # Create outputs with expected shapes
    # scalar: [3,4,5] with scalar index -> [4,5]
    output_scalar = helper.make_tensor_value_info(
        "output_scalar", TensorProto.FLOAT, [4, 5]
    )
    ...
    output_2d = helper.make_tensor_value_info(
        "output_2d", TensorProto.FLOAT, [1, 2, 4, 5]
    )
    ...
    assert output_scalar.shape == (4, 5), (
        f"Scalar output shape {output_scalar.shape} != (4, 5)"
    )
    assert output_1d.shape == (1, 4, 5), (
        f"1D output shape {output_1d.shape} != (1, 4, 5)"
    )
    assert output_2d.shape == (1, 2, 4, 5), (
        f"2D output shape {output_2d.shape} != (1, 2, 4, 5)"
    )
