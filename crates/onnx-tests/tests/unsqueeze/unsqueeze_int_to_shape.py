...
def create_unsqueeze_int_to_shape_model():
    ...
    unsqueeze_node = helper.make_node(
        "Unsqueeze", inputs=["scalar_input", "axes"], outputs=["shape_output"]
    )
...
def create_squeeze_unsqueeze_roundtrip_model():
    ...
    input_shape = helper.make_tensor_value_info("shape_input", TensorProto.INT64, [1])

    # Output: Shape[1] after roundtrip
    output_shape = helper.make_tensor_value_info("shape_output", TensorProto.INT64, [1])
    ...
    squeeze_axes_const = helper.make_node(
        ...
        value=helper.make_tensor(
            name="squeeze_axes_value", data_type=TensorProto.INT64, dims=[1], vals=[0]
        ),
    )
    ...
    unsqueeze_axes_const = helper.make_node(
        ...
        value=helper.make_tensor(
            name="unsqueeze_axes_value", data_type=TensorProto.INT64, dims=[1], vals=[0]
        ),
    )
...
def main():
    ...
    try:
        ...
        assert np.array_equal(test_input2, output2), (
            "Roundtrip failed to preserve value"
        )
