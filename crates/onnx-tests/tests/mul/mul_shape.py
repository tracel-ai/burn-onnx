...
def main():
    ...
    input_tensor2 = helper.make_tensor_value_info(
        "input2", TensorProto.FLOAT, [1, 2, 3]
    )
    ...
    scalar_const = helper.make_node(
        ...
        value=helper.make_tensor(
            name="const_tensor", data_type=TensorProto.INT64, dims=[], vals=[2]
        ),
    )

    # Multiply shape by scalar (Shape * Scalar)
    mul_scalar_node = helper.make_node(
        "Mul", inputs=["shape1", "scalar"], outputs=["shape_times_scalar"]
    )

    # Multiply shape by shape (Shape * Shape)
    mul_shapes_node = helper.make_node(
        "Mul", inputs=["shape1", "shape2"], outputs=["shape_times_shape"]
    )

    # Outputs - shape arrays
    output1 = helper.make_tensor_value_info(
        "shape_times_scalar", TensorProto.INT64, [3]
    )
