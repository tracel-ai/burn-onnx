...
def main():
    ...
    input_tensor = helper.make_tensor_value_info(
        "input_tensor", TensorProto.FLOAT, [3, 4, 5]
    )
    ...
    mul_shape_tensor = helper.make_node(
        "Mul", inputs=["shape", "input_1d"], outputs=["shape_times_tensor"]
    )

    # Multiply tensor with shape (Tensor * Shape) - reversed order
    mul_tensor_shape = helper.make_node(
        "Mul", inputs=["input_1d", "shape"], outputs=["tensor_times_shape"]
    )

    # Outputs
    output1 = helper.make_tensor_value_info(
        "shape_times_tensor", TensorProto.INT64, [3]
    )
    output2 = helper.make_tensor_value_info(
        "tensor_times_shape", TensorProto.INT64, [3]
    )
    ...
    assert np.array_equal(shape_times_tensor, tensor_times_shape), (
