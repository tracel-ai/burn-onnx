...
def main():
    ...
    shape1_node = helper.make_node(
        "Shape", inputs=["input"], outputs=["shape1"], name="shape1_node"
    )

    # Second Shape: Get shape of shape1 (returns rank)
    rank_shape_node = helper.make_node(
        "Shape", inputs=["shape1"], outputs=["rank_shape"], name="rank_shape_node"
    )
    ...
    # Create outputs
    shape1_output = helper.make_tensor_value_info("shape1", TensorProto.INT64, [4])
    ...
    graph = helper.make_graph(
        ...
        [
            shape1_output,
            rank_shape_output,
            partial_shape_output,
            partial_rank_shape_output,
        ],
    )
