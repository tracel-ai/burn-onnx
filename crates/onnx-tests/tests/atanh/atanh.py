...
# used to generate model: atanh.onnx
        ...
        graph=onnx.helper.make_graph(
            ...
            nodes=[
                onnx.helper.make_node(
                    "Atanh", inputs=["input1"], outputs=["output1"], name="/Atanh"
                ),
            ],
            inputs=[
...
def main():
    ...
    # atanh domain is (-1, 1)
    test_input = np.array([[[[-0.5, 0.0, 0.5, 0.9]]]], dtype=np.float32)
