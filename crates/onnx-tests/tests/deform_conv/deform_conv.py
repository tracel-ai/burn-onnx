...
# used to generate model: deform_conv.onnx
...
def main():
    ...
    # Shapes: X=[1,1,3,3], W=[1,1,2,2], offset=[1,8,2,2]
    # oH = (3 - 2) / 1 + 1 = 2, oW = 2
    # offset channels = 2 * offset_group * kH * kW = 2 * 1 * 2 * 2 = 8
    ...
    graph = helper.make_graph([node], "deform_conv_graph", [X, offset], [Y], initializer=[W])
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 19)])
    model.ir_version = 9
    onnx.checker.check_model(model)

    file_name = "deform_conv.onnx"
    ...
    print("Test offset shape: {}".format(test_offset.shape))
