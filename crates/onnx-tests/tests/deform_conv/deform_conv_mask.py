...
# used to generate model: deform_conv_mask.onnx
...
def main():
    ...
    # Shapes: X=[1,1,3,3], W=[1,1,2,2], offset=[1,8,2,2], B=[1], mask=[1,4,2,2]
    # mask channels = offset_group * kH * kW = 1 * 2 * 2 = 4
    ...
    mask = helper.make_tensor_value_info("mask", TensorProto.FLOAT, [1, 4, 2, 2])
    ...
    graph = helper.make_graph(
        [node], "deform_conv_mask_graph", [X, offset, mask], [Y], initializer=[W, B]
    )
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 19)])
    model.ir_version = 9
    onnx.checker.check_model(model)

    file_name = "deform_conv_mask.onnx"
    ...
    test_mask = np.ones([1, 4, 2, 2], dtype=np.float32)

    ref = ReferenceEvaluator(model)
    [output] = ref.run(None, {"X": test_x, "offset": test_offset, "mask": test_mask})
