...
# used to generate model: elu.onnx
...
def main():
    # Build graph: Y = Elu(X, alpha=0.5)
    X = helper.make_tensor_value_info("X", TensorProto.FLOAT, [2, 3])
    Y = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 3])

    elu_node = helper.make_node("Elu", inputs=["X"], outputs=["Y"], alpha=0.5)

    graph = helper.make_graph([elu_node], "elu_graph", [X], [Y])
