...
# used to generate model: selu_custom.onnx (alpha=2, gamma=3)
...
def main():
    # Build graph: Y = Selu(X) with alpha=2, gamma=3
    X = helper.make_tensor_value_info("X", TensorProto.FLOAT, [2, 3])
    Y = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [2, 3])

    node = helper.make_node("Selu", inputs=["X"], outputs=["Y"], alpha=2.0, gamma=3.0)
