...
def main():
    ...
    identity1_node = helper.make_node(
        "Identity", inputs=["input"], outputs=["identity1_out"], name="identity1"
    )
    ...
    relu_node = helper.make_node(
        "Relu", inputs=["identity2_out"], outputs=["final_output"], name="relu_final"
    )
    ...
    model = helper.make_model(
        graph, producer_name="test", opset_imports=[helper.make_operatorsetid("", 16)]
    )
