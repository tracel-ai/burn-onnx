...
class InterpolateModel(nn.Module):
    def __init__(
        self, scale_factor=None, size=None, mode="nearest", align_corners=None
    ):
...
if __name__ == "__main__":
    ...
    export_interpolate_onnx(
        "resize_1d_nearest_scale.onnx", scale_factor=1.5, mode="nearest", rank=1
    )
    export_interpolate_onnx(
        "resize_1d_linear_scale.onnx",
        scale_factor=1.5,
        mode="linear",
        rank=1,
        align_corners=True,
    ...
    # 2D (spatial) examples
    export_interpolate_onnx(
        "resize_2d_nearest_scale.onnx", scale_factor=1.5, mode="nearest", rank=2
    )
    export_interpolate_onnx(
        "resize_2d_bilinear_scale.onnx",
        scale_factor=1.5,
        mode="bilinear",
        rank=2,
        align_corners=True,
