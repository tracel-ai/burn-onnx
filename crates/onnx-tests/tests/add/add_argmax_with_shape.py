...
def main():
    ...
    input_tensor = helper.make_tensor_value_info(
        "input_tensor", TensorProto.FLOAT, [3, 4]
    )
    ...
    const_one = helper.make_node(
        ...
        value=helper.make_tensor(
            name="const_one", data_type=TensorProto.INT64, dims=[], vals=[1]
        ),
    )

    gather_node = helper.make_node(
        "Gather", inputs=["shape", "one"], outputs=["shape_dim"], axis=0
    )
    ...
    const_zero = helper.make_node(
        ...
        outputs=["zero"],
        value=helper.make_tensor(
            name="const_zero", data_type=TensorProto.INT64, dims=[], vals=[0]
        ),
    )
    ...
    const_three = helper.make_node(
        ...
        outputs=["three"],
        value=helper.make_tensor(
            name="const_three", data_type=TensorProto.INT64, dims=[], vals=[3]
        ),
    )

    range_node = helper.make_node(
        "Range", inputs=["zero", "three", "one"], outputs=["range_result"]
    )

    # Multiply range with shape dimension (produces shape type)
    mul_node = helper.make_node(
        "Mul", inputs=["range_result", "shape_dim"], outputs=["mul_result"]
    )

    # Add argmax result (tensor) with multiplication result (shape)
    # This is the operation that was failing
    add_node = helper.make_node(
        "Add", inputs=["argmax_result", "mul_result"], outputs=["final_result"]
    )
    ...
    test_input = np.array(
        [[1.0, 3.0, 2.0, 4.0], [5.0, 2.0, 6.0, 3.0], [2.0, 4.0, 1.0, 5.0]],
        dtype=np.float32,
    )
