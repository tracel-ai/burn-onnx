...
def main():
    ...
    input_tensor2 = helper.make_tensor_value_info(
        "input2", TensorProto.FLOAT, [5, 6, 7]
    )
    ...
    scalar_const = helper.make_node(
        ...
        value=helper.make_tensor(
            name="const_tensor", data_type=TensorProto.INT64, dims=[], vals=[10]
        ),
    )

    # Add shape with scalar (Shape + Scalar)
    add_scalar_node = helper.make_node(
        "Add", inputs=["shape1", "scalar"], outputs=["shape_plus_scalar"]
    )

    # Add two shapes together (Shape + Shape)
    add_shapes_node = helper.make_node(
        "Add", inputs=["shape1", "shape2"], outputs=["shape_plus_shape"]
    )
