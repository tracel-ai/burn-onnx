...
def build_model():
    ...
    # Outputs: [sum_out]
    scan_add = helper.make_node("Add", inputs=["sum_in", "elem"], outputs=["sum_out"])
    ...
    inner_then_identity = helper.make_node(
        "Identity", inputs=["x_inner"], outputs=["scan_sequence"]
    )
    ...
    inner_else_sub = helper.make_node(
        "Sub", inputs=["x_inner", "one_const"], outputs=["else_out"]
    )
    ...
    loop_identity_x = helper.make_node(
        "Identity", inputs=["accum"], outputs=["x_inner"]
    )

    # Pass cond2 through unchanged
    loop_identity_cond2 = helper.make_node(
        "Identity", inputs=["cond2"], outputs=["cond2_out"]
    )
    ...
    outer_then_loop = helper.make_node(
        ...
        outputs=[
            "loop_result",
            "cond2_final",
        ],  # Must output all 2 loop-carried dependencies
    ...
    outer_then_graph = helper.make_graph(
        ...
        inputs=[
            ...
            helper.make_tensor_value_info(
                "iterations", TensorProto.INT64, []
            ),  # Rank-0 (scalar)
            helper.make_tensor_value_info(
                "cond_init", TensorProto.BOOL, []
            ),  # Rank-0 (scalar)
        ],
        outputs=[
    ...
    outer_else_mul = helper.make_node(
        "Mul", inputs=["x_init", "two_const"], outputs=["else_result"]
    )
    ...
    outer_else_graph = helper.make_graph(
        ...
        outputs=[
            helper.make_tensor_value_info("else_result", TensorProto.FLOAT, [2, 3])
        ],
        initializer=[
            numpy_helper.from_array(np.array([2.0], dtype=np.float32), name="two_const")
        ],
    )
    ...
    main_graph = helper.make_graph(
        ...
        inputs=[
            ...
            helper.make_tensor_value_info(
                "condition1", TensorProto.BOOL, []
            ),  # Rank-0 (scalar)
            helper.make_tensor_value_info(
                "condition2", TensorProto.BOOL, []
            ),  # Rank-0 (scalar)
        ],
