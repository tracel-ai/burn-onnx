...
mod tests {
    ...
    #[test]
    fn batch_norm() {
        let model: batch_norm::Model = batch_norm::Model::default();

        // Run the model with ones as input for easier testing
        let input = Tensor::<3>::ones([1, 20, 1], &Default::default());
        ...
        let expected_sum: f32 = 19.999_802; // from pytorch
        assert!(expected_sum.approx_eq(output_sum, (1.0e-8, 2)));
    }

    #[test]
    fn batch_norm_runtime() {
        let model: batch_norm_runtime::Model = batch_norm_runtime::Model::default();
        ...
        let scale = Tensor::<1>::from_floats([1.0, 2.0, 0.5], &device);
        let bias = Tensor::<1>::from_floats([0.0, 1.0, -1.0], &device);
        let mean = Tensor::<1>::from_floats([0.0, 0.5, -0.5], &device);
        let var = Tensor::<1>::from_floats([1.0, 2.0, 0.5], &device);
    ...
    fn batch_norm_partial_constant() {
        let model: batch_norm_partial_constant::Model =
