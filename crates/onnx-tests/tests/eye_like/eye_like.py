...
def build_eye_like_model():
    ...
    output_tensor = helper.make_tensor_value_info(
        "output", TensorProto.FLOAT, ["H", "W"]
    )

    eye_like_node = helper.make_node(
        "EyeLike", inputs=["input"], outputs=["output"], name="EyeLike_0"
    )

    graph = helper.make_graph(
        [eye_like_node], "EyeLikeModel", [input_tensor], [output_tensor]
    )
...
def build_eye_like_k_minus1_model():
    ...
    eye_like_node = helper.make_node(
        "EyeLike", inputs=["input"], outputs=["output"], name="EyeLike_k_minus1", k=-1
    )

    graph = helper.make_graph(
        [eye_like_node], "EyeLikeKMinus1Model", [input_tensor], [output_tensor]
    )
...
def build_eye_like_float64_model():
    ...
    graph = helper.make_graph(
        [eye_like_node], "EyeLikeFloat64Model", [input_tensor], [output_tensor]
    )
...
def build_eye_like_int32_model():
    ...
    graph = helper.make_graph(
        [eye_like_node], "EyeLikeInt32Model", [input_tensor], [output_tensor]
    )
...
def build_eye_like_bool_model():
    ...
    graph = helper.make_graph(
        [eye_like_node], "EyeLikeBoolModel", [input_tensor], [output_tensor]
    )
...
def build_eye_like_large_k_model():
    ...
    graph = helper.make_graph(
        [eye_like_node], "EyeLikeLargeKModel", [input_tensor], [output_tensor]
    )
...
def build_eye_like_small_matrix_model():
    ...
    eye_like_node = helper.make_node(
        "EyeLike", inputs=["input"], outputs=["output"], name="EyeLike_1x1"
    )

    graph = helper.make_graph(
        [eye_like_node], "EyeLike1x1Model", [input_tensor], [output_tensor]
    )
...
def build_eye_like_wide_matrix_model():
    ...
    eye_like_node = helper.make_node(
        "EyeLike", inputs=["input"], outputs=["output"], name="EyeLike_wide", k=0
    )

    graph = helper.make_graph(
        [eye_like_node], "EyeLikeWideModel", [input_tensor], [output_tensor]
    )
...
def build_eye_like_negative_large_k_model():
    ...
    graph = helper.make_graph(
        [eye_like_node], "EyeLikeNegLargeKModel", [input_tensor], [output_tensor]
    )
