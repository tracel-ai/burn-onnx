...
def main():
    ...
    shape_x = helper.make_node("Shape", inputs=["input3"], outputs=["shape_x"])

    shape_y = helper.make_node("Shape", inputs=["input4"], outputs=["shape_y"])
    ...
    shape_equal = helper.make_node(
        "Equal", inputs=["shape1", "shape2"], outputs=["condition_shape"]
    )

    # Where operation with all Shape types
    # Where(condition_shape, shape_x, shape_y)
    where_node = helper.make_node(
        "Where", inputs=["condition_shape", "shape_x", "shape_y"], outputs=["output"]
    )
