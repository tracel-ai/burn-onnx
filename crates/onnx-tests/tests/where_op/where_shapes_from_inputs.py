...
def main():
    ...
    input4 = helper.make_tensor_value_info(
        "input4", TensorProto.FLOAT, [1, 0, 3]
    )  # For comparison

    # Shape nodes - extract shapes of inputs
    shape1 = helper.make_node("Shape", inputs=["input1"], outputs=["shape1"])

    shape2 = helper.make_node("Shape", inputs=["input2"], outputs=["shape2"])

    shape3 = helper.make_node("Shape", inputs=["input3"], outputs=["shape3"])

    shape4 = helper.make_node("Shape", inputs=["input4"], outputs=["shape4"])
