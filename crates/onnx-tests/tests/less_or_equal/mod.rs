...
mod tests {
    ...
    fn less_or_equal() {
        ...
        let model: less_or_equal::Model = less_or_equal::Model::new(&device);
    ...
    fn less_or_equal_scalar() {
        let device = Default::default();
        let model: less_or_equal_scalar::Model = less_or_equal_scalar::Model::new(&device);
    ...
    fn less_or_equal_broadcast() {
        let device = Default::default();
        let model: less_or_equal_broadcast::Model = less_or_equal_broadcast::Model::new(&device);

        // Shape [1] vs [4, 4] - scalar-like broadcast
        let input1 = Tensor::<1>::from_floats([2.5], &device);
    ...
    #[test]
    fn less_or_equal_shape_operand_with_rank4_tensor() {
        // dim0 of x's shape (B = 2) is a length 1 Shape, lifted to rank 4 before the comparison.
        // Outputs are `x <= 2` and `2 <= x`.
        let device = Default::default();
        let model: less_or_equal_shape_rank_lift::Model =
            less_or_equal_shape_rank_lift::Model::from_file(
                concat!(env!("OUT_DIR"), "/model/less_or_equal_shape_rank_lift.bpk"),
                &device,
            );
