...
def create_shape_start_tensor_end_model():
    ...
    input_tensor = helper.make_tensor_value_info("input", TensorProto.FLOAT, [6, 8, 10])
    shape_tensor = helper.make_tensor_value_info(
        "shape_input",
        TensorProto.FLOAT,
        [2, 3],  # Will use shape [2, 3] as starts
    )
    end_tensor = helper.make_tensor_value_info(
        "ends",
        TensorProto.INT64,
        [2],  # 1D tensor with 2 elements for ends
    )
    output_tensor = helper.make_tensor_value_info(
        "output",
        TensorProto.FLOAT,
        [3, 5, 10],  # expected output shape
    )
    ...
    shape_node = helper.make_node(
        "Shape", inputs=["shape_input"], outputs=["shape"], name="shape_op"
    )
    ...
    outputs = session.run(
        None, {"input": input_data, "shape_input": shape_input_data, "ends": ends}
    )

    print("  Model: slice_shape_start_tensor_end.onnx")
    print(
        f"  Inputs: input{list(input_data.shape)}, shape_input{list(shape_input_data.shape)}, ends{list(ends.shape)}"
    )
...
def create_tensor_start_shape_end_model():
    ...
    start_tensor = helper.make_tensor_value_info(
        "starts",
        TensorProto.INT64,
        [2],  # 1D tensor with 2 elements for starts
    )
    shape_tensor = helper.make_tensor_value_info(
        "shape_input",
        TensorProto.FLOAT,
        [6, 10],  # Will use shape [6, 10] as ends
    )
    output_tensor = helper.make_tensor_value_info(
        "output",
        TensorProto.FLOAT,
        [4, 7, 8],  # expected output shape
    )
    ...
    outputs = session.run(
        None, {"input": input_data, "starts": starts, "shape_input": shape_input_data}
    )

    print("  Model: slice_tensor_start_shape_end.onnx")
    print(
        f"  Inputs: input{list(input_data.shape)}, starts{list(starts.shape)}, shape_input{list(shape_input_data.shape)}"
    )
