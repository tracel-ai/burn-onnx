...
def build_manual_graph():
    ...
    input_tensor = helper.make_tensor_value_info("input", TensorProto.FLOAT, [10, 8, 6])

    # Define a shape tensor that will provide slice parameters
    shape_tensor = helper.make_tensor_value_info(
        "shape_input",
        TensorProto.FLOAT,
        [3, 4],  # Will use its shape [3, 4] as slice params
    )
    ...
    output_tensor = helper.make_tensor_value_info(
        "output",
        TensorProto.FLOAT,
        [3, 4, 6],  # Slicing [10,8,6] from [0:3, 0:4, :]
    )
    ...
    shape_node = helper.make_node(
        "Shape", inputs=["shape_input"], outputs=["shape"], name="shape"
    )
    ...
    starts_node = helper.make_node(
        ...
        value=helper.make_tensor(
            name="starts_value", data_type=TensorProto.INT64, dims=[2], vals=[0, 0]
        ),
    )
...
def main():
    ...
    try:
        ...
        (actual_output,) = session.run(
            None, {"input": input_data, "shape_input": shape_input_data}
        )
        ...
        print(
            f"Slice operation: input[0:{shape[0]}, 0:{shape[1]}, :] -> output[{actual_output.shape}]"
        )
