...
def create_slice_axes_model():
    ...
    output_tensor = helper.make_tensor_value_info(
        "output", TensorProto.FLOAT, [2, 2, 6]
    )
    ...
    slice_node = helper.make_node(
        "Slice", inputs=["input", "starts", "ends", "axes"], outputs=["output"]
    )
...
def main():
    ...
    print(
        f"- Slice parameters: starts=[{starts_val}], ends=[{ends_val}], axes=[{axes_val}]"
    )
    print(f"- Output shape: {list(output.shape)}")
    print(
        f"- Effect: Slices dimension {axes_val} from index {starts_val} to {ends_val}"
    )
