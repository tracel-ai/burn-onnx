...
def main():
    ...
    input_tensor = helper.make_tensor_value_info("input", TensorProto.FLOAT, [4, 5, 6])
    start_tensor = helper.make_tensor_value_info(
        "starts",
        TensorProto.INT64,
        [2],  # 1D tensor with 2 elements
    )
    ...
    outputs = session.run(None, {"input": input_data, "starts": starts, "ends": ends})

    print("Finished exporting model to slice_1d_tensor.onnx")
    print("Model structure:")
    print(
        f"  Inputs: input{list(input_data.shape)}, starts{list(starts.shape)}, ends{list(ends.shape)}"
    )
