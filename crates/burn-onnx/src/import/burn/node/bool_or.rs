    ...
    fn forward(&self, scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let function = match (&lhs_arg.ty, &rhs_arg.ty) {
            (lhs_ty, rhs_ty) if lhs_ty.is_on_device() && rhs_ty.is_on_device() => {
                ...
                quote! { #lhs_bc.bool_or(#rhs_bc) }
            }
            (ArgType::ScalarNative(_), rhs_ty) if rhs_ty.is_on_device() => {
                let rank = rhs_ty.rank();
                quote! {
                    if #lhs { Tensor::<#rank, Int>::ones(#rhs.shape(), &self.device).bool() } else { #rhs }
                }
            }
            (lhs_ty, ArgType::ScalarNative(_)) if lhs_ty.is_on_device() => {
                let rank = lhs_ty.rank();
                quote! {
                    if #rhs { Tensor::<#rank, Int>::ones(#lhs.shape(), &self.device).bool() } else { #lhs }
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_or_tensor_scalar_forward() {
        let node = OrNodeBuilder::new("or1")
        ...
        pub fn forward(&self, lhs: [i64; 4], rhs: [i64; 4]) -> [i64; 4] {
            let output = {
                ...
                core::array::from_fn::<
                    ...
                    4usize,
                    _,
                >(|__i| if __lhs[__i] != 0 || __rhs[__i] != 0 { 1i64 } else { 0i64 })
            };
            output
        }
        ");
        ...
        pub fn forward(&self, lhs: [i64; 1], rhs: [i64; 4]) -> [i64; 4] {
            let output = {
                ...
                core::array::from_fn::<
                    ...
                    4usize,
                    _,
                >(|__i| if __lhs[0] != 0 || __rhs[__i] != 0 { 1i64 } else { 0i64 })
            };
            output
        }
        ");
        ...
        pub fn forward(&self, lhs: [i64; 4], rhs: [i64; 1]) -> [i64; 4] {
            let output = {
                ...
                core::array::from_fn::<
                    ...
                    4usize,
                    _,
                >(|__i| if __lhs[__i] != 0 || __rhs[0] != 0 { 1i64 } else { 0i64 })
            };
            output
        }
        ");
