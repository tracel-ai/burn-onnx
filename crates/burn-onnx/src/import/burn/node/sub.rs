    ...
    fn forward(&self, scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let function = match (&lhs_arg.ty, &rhs_arg.ty) {
            ...
            (lhs_ty, ArgType::Shape(_)) if lhs_ty.is_on_device() => {
                ...
                quote! {
                    #lhs.sub(Tensor::<1, burn::tensor::Int>::from_data(
    ...
    fn test_shape_shape() {
        ...
        pub fn forward(&self, lhs: [i64; 4], rhs: [i64; 4]) -> [i64; 4] {
            let output = {
                ...
                for __i in 0..4usize {
                    __result[__i] = (__lhs[__i]).saturating_sub(__rhs[__i]);
                }
                __result
            };
            output
    ...
    fn test_shape_shape_broadcast_lhs() {
        ...
        pub fn forward(&self, lhs: [i64; 1], rhs: [i64; 4]) -> [i64; 4] {
            let output = {
                ...
                #[allow(clippy::needless_range_loop)]
                for __i in 0..4usize {
                    __result[__i] = (__lhs[0]).saturating_sub(__rhs[__i]);
                }
                __result
            };
            output
    ...
    fn test_shape_shape_broadcast_rhs() {
        ...
        pub fn forward(&self, lhs: [i64; 4], rhs: [i64; 1]) -> [i64; 4] {
            let output = {
                ...
                #[allow(clippy::needless_range_loop)]
                for __i in 0..4usize {
                    __result[__i] = (__lhs[__i]).saturating_sub(__rhs[0]);
                }
                __result
            };
            output
