    ...
    fn forward(&self, scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let function = match (&lhs.ty, &rhs.ty) {
            ...
            (lhs_ty, ArgType::ScalarNative(_)) if lhs_ty.is_on_device() => {
                quote! { #lhs_value.bitwise_xor_scalar(#rhs_value as i64) }
            }
            (ArgType::ScalarNative(_), rhs_ty) if rhs_ty.is_on_device() => {
                quote! { #rhs_value.bitwise_xor_scalar(#lhs_value as i64) }
            }
...
mod tests {
    ...
    fn test_bitwisexor_scalar() {
        ...
        pub fn forward(&self, lhs: Tensor<2, Int>, rhs: i32) -> Tensor<2, Int> {
            let output = lhs.bitwise_xor_scalar(rhs as i64);
            output
        }
        ");
    }
    ...
    fn test_bitwisexor_scalar_tensor() {
        ...
        pub fn forward(&self, lhs: i32, rhs: Tensor<2, Int>) -> Tensor<2, Int> {
            let output = rhs.bitwise_xor_scalar(lhs as i64);
            output
        }
        ");
    }
