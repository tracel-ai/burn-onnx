    ...
    fn forward(&self, _scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let size_expr = match &self.config.size {
            ...
            WindowSize::Runtime(runtime_ref) => {
                ...
                quote! { {
                    assert!(#name >= 0, "HammingWindow: size must be non-negative, got {}", #name);
                    #name as usize
                } }
            }
        };
        ...
        quote! {
            let #output = hamming_window(#size_expr, #periodic, &self.device)
                .cast(#output_dtype);
        }
    }
...
mod tests {
    ...
    fn test_hamming_window_static() {
        ...
        pub fn forward(&self) -> Tensor<1> {
            let output = hamming_window(10usize, true, &self.device)
    ...
    fn test_hamming_window_symmetric() {
        ...
        assert_snapshot!(code, @r"
        pub fn forward(&self) -> Tensor<1> {
            let output = hamming_window(8usize, false, &self.device)
    ...
    fn test_hamming_window_runtime() {
        ...
        pub fn forward(&self, size: i64) -> Tensor<1> {
            let output = hamming_window(
                    {
                        assert!(
                            size >= 0, "HammingWindow: size must be non-negative, got {}", size
                        );
                        size as usize
                    },
                    true,
